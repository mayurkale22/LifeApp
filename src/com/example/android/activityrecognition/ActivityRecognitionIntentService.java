...
public class ActivityRecognitionIntentService extends IntentService  {
    ...
    public ActivityRecognitionIntentService(Context context) {
        // Set the label for the service's background thread
        super("ActivityRecognitionIntentService");
    ...
    private void logActivityRecognitionResult(ActivityRecognitionResult result) {
        ...
        for (DetectedActivity detectedActivity : result.getProbableActivities()) {
            ...
            */
            ...
            //nav code
            //if(confidence>=LogFile.getInstance(getApplicationContext()).get_current_activity_confidence())
            if(confidence>=50)
            	{
            	//LogFile.getInstance(getApplicationContext()).set_current_activity(activityName);
                //LogFile.getInstance(getApplicationContext()).set_current_activity_confidence(confidence);
                //LogFile.getInstance(getApplicationContext()).set_current_activity_timestamp(timeStamp);
                
                Transfer.setCurrentactivity(activityName);
                Transfer.set_current_activity_confidence(confidence);
                Transfer.set_current_activity_timestamp(timeStamp);
                System.out.println("higher confidence detected");
                
            	}
                       
            
            Log.i("ActREC", "ACTIVITY NAME : "+activityName+" CONFIDENCE : "+confidence);
            System.out.println("ACTIVITY NAME : "+activityName+" CONFIDENCE : "+confidence);
            
        	//TextView tv=(TextView) ((MainActivity)context).findViewById(R.id.live); //--nav code
            //tv.setText("ACTIVITY NAME : "+activityName+" CONFIDENCE : "+confidence);
